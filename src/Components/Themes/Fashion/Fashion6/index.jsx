    ...
    <>
      ...
      {data?.products_list_1?.status && (
        <>
          ...
          <WrapperComponent classes={{ sectionClass: "section-b-space pt-0 ratio_square", fluidClass: "container" }}>{data?.products_list_1?.product_ids?.length ? <HomeProduct productIds={data?.products_list_1?.product_ids} style="vertical" /> : <NoDataFound customClass={"no-data-added"} title={"No Product Found"} />}</WrapperComponent>
        </>
      )}
