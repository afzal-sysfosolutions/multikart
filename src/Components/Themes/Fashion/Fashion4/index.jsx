        ...
        <WrapperComponent classes={{ sectionClass: "container section-b-space fashion-product-section" }} customCol={true}>
          ...
          {data?.products_list?.products?.status && (
            <div className={data?.products_list?.categories?.status ? "col-xl-10" : "col-xl-12"}>
              ...
              {data?.products_list?.products?.product_ids?.length  ?    <HomeProduct productIds={data?.products_list?.products?.product_ids || []} slider={false} style="vertical" />  :<NoDataFound customClass={'no-data-added'} title={"No Product Found"}/>}
              </Container>
            </div>
          )}
        </WrapperComponent>
