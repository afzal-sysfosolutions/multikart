...
import { useFatchCollection } from "@/Utils/Services/FatchCollection";
...
const CollectionProducts = ({ filter, grid, infiniteScroll, categorySlug }) => {
  ...
  const {ApiCateCalling} = useCateCollection()
  const {CollectionApiCalling} = useFatchCollection()
  ...
  const FetchCategoryData = async(ID)=>{
    ...
    const OBJ ={
     CategoryId:ID, ClientId: '1', CookieId:CookieID, FilterBy:'Priority', Pagination:0, ProductName:'', ProductTagId:0,
    }
  ...
  useEffect(()=>{
   if(slug?.category) FetchCategoryData(slug?.category[1])
  },[slug?.category, param])
    ...
    <Suspense fallback={<div>Loading...</div>}>
      <div
        className={`product-wrapper-grid ${
          infiniteScroll ? "product-load-more" : ""
        } ${grid == "list" ? "list-view" : ""} ${
      ...
      >
        <Row className="g-xl-4 g-lg-3 g-sm-4 g-3">
        {/* {PageData?.map((item, index) => <div className={adjustGrid} key={index}>
            <div className="basic-product theme-product-1 ">
              <div className="overflow-hidden">
                <div className="img-wrapper">
                  <Link href={'/product/'+item?.ProductName.split(" ").join('-')+'/'+item?.ProductId+'/'+item?.ProductVariantDetailId}>
                    <img
                      src={process.env.NEXT_PUBLIC_PRODUCT_IMG + item?.ProductImage ||  "/assets/images/placeholder/product.png"}
                ...
                <div className="product-detail">
                  <div>
                    <div className="brand-w-color">
                      <a className="product-title" href="/brand/glamour-gaze">
                        {item?.Brand || 'Even'}
                      </a>
                    ...
                    <h4 className="price">
                    ₹{item?.ProductPrice}<del>₹{item?.UsualPrice}</del>
                      <span className="discounted-price">{calculateDiscountPercentage(item?.UsualPrice, item?.ProductPrice)} Off</span>
                    </h4>
                  </div>
                  <ul className="offer-panel">
                    <li>
                      ...
                      Limited Time Offer: {calculateDiscountPercentage(item?.UsualPrice, item?.ProductPrice)} off
                    </li>
                    <li>
                      <span className="offer-icon">
                        <svg
                      ...
                      Limited Time Offer: {calculateDiscountPercentage(item?.UsualPrice, item?.ProductPrice)} 
                    </li>
                    <li>
                      <span className="offer-icon">
                        <svg
          ...
          {/* <React.Fragment>
              {MenCollection?.KurtaPajama?.map((item, index) => (
                <Col className={adjustGrid} key={index}>
                  <ProductBox product={item} style="vertical" />
                </Col>
              ))}
